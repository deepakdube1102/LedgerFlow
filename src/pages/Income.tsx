...
import { ArrowDownCircle, Search, Filter, MoreHorizontal, Edit, Trash2, X, Plus } from "lucide-react";
...
export default function Income() {
    ...
    const defaultForm = { source: "", amount: "", date: new Date().toISOString().split('T')[0], category: "Salary", notes: "" };
    ...
    const saveMutation = useMutation({
        mutationFn: async (data: typeof formData) => {
            ...
            const url = isEdit ? `/api/income/${editingRecord._id}` : '/api/income';
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show" className="max-w-7xl mx-auto space-y-6 relative">
            <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="font-heading text-2xl font-bold text-foreground">Income</h2>
                    <p className="text-muted-foreground text-sm mt-1">Track and manage all your revenue sources.</p>
                </div>
                <button
            ...
            <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="dashboard-card flex items-center gap-4 border-l-4 border-primary">
                    <div className="w-12 h-12 rounded-xl kpi-gradient-primary flex items-center justify-center">
                        <ArrowDownCircle className="w-6 h-6 text-white" />
                    </div>
                    <div>
                        <p className="text-sm text-muted-foreground">Total Income</p>
                        <p className="text-2xl font-heading font-bold text-foreground">${totalIncome.toLocaleString()}</p>
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div variants={item} className="dashboard-card !p-0 overflow-hidden">
                <div className="p-4 border-b border-border flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="relative w-full sm:w-64">
                        ...
                        <input className="pl-9 pr-4 py-2 w-full rounded-lg bg-secondary text-sm text-foreground placeholder:text-muted-foreground outline-none" placeholder="Search income..." />
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {isLoading ? (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center text-muted-foreground">Loading income records...</td>
                                </tr>
                            ) : incomeRecords?.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center text-muted-foreground">No income records found. Add your first entry!</td>
                                </tr>
                                ...
                                <tr key={record._id} className="border-b border-border last:border-0 hover:bg-secondary/50 transition-colors">
                                    ...
                                    <td className="p-4 font-medium text-foreground">{record.source}</td>
                                    <td className="p-4">
                ...
                <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-1">Source / Client Name</label>
                                <input
                                    type="text" required
